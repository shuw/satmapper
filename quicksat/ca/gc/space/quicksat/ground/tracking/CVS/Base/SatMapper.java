...
public class SatMapper extends JPanel implements Runnable, KeyListener, MouseMotionListener, MouseListener {
    ...
    public void moveScreen(double longi, double lat) {
        moveScreen( getXCoord(longi) + mapImgOffScreenX, getYCoord(lat) + mapImgOffScreenY );
    }
    ...
    private void drawDate(Graphics g) {
        ...
        if (langEng) {
            ...
            if ( Math.abs(myData.timeJump) > 0) {
                if (myData.timeJump > 0)
                    g.drawString("Temps avancé" , 5 , y + 12 );
                else
                    g.drawString("Temps passé" , 5 , y + 12 );
            }
        }
    }
